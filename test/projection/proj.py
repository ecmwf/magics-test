...
def plot_area(epsg, area, index):
	...
	title = "Projection {} : {}".format(epsg, area)
...
index = 0
for epsg in ["EPSG:32661", "EPSG:32761"]:
	for i in range(1, 20):
		index = index + 1
		plot_area(epsg, random_area(epsg), index)


# 	llx, lly = revert.transform(xmin, ymin)
# 	urx, ury = revert.transform(xmax, ymax)
